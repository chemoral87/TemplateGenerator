...
#set($firstColumn = ${element.column_name})
      ...
      #if ( $velocityCount != 1 ) ,#else  #end{key: "${element.column_name}", updatable: "hidden", deletable: true }
#elseif ( $element.data_type == "date" )
      #if ( $velocityCount != 1 ) ,#else  #end{key:"${element.column_name}", label: "${element.column_title}", type:"date", insertable: true, updatable: true #if( $element.is_nullable == "NO") ,nullable: false #else #end}
#elseif ( $element.data_type == "datetime" )
      #if ( $velocityCount != 1 ) ,#else  #end{key:"${element.column_name}", label: "${element.column_title}", type:"datetime", insertable: true, updatable: true #if( $element.is_nullable == "NO") ,nullable: false #else #end}
#elseif ( $element.data_type == "int" )
      #if ( $velocityCount != 1 ) ,#else  #end{key:"${element.column_name}", label: "${element.column_title}", type:"integer", insertable: true, updatable: true, parser:"number" #if( $element.is_nullable == "NO") ,nullable: false #else #end}   
#elseif ( $element.data_type == "numeric" || $element.data_type == "decimal" )
      #if ( $velocityCount != 1 ) ,#else  #end{key:"${element.column_name}", label: "${element.column_title}", type:"decimal", insertable: true, updatable: true, mDec:${element.numeric_scale}, vMax: (Math.pow(10,(${element.numeric_precision}-${element.numeric_scale})) - (1 / Math.pow(10,${element.numeric_scale}))), parser:"number" #if( $element.is_nullable == "NO") ,nullable: false #else #end}   
#elseif ( $element.data_type == "varchar" || $element.data_type == "nvarchar" ) 
      #if ( $velocityCount != 1 ) ,#else  #end{key:"${element.column_name}", label: "${element.column_title}", type:"varchar", insertable: true, updatable: true,  maxlength: ${element.character_maximum_length} #if( $element.is_nullable == "NO") ,nullable: false #else #end}
#elseif ( $element.data_type == "bit" ) 
      #if ( $velocityCount != 1 ) ,#else  #end{key:"${element.column_name}", label: "${element.column_title}", type:"bit", insertable: true, updatable: true }	  
#end
#end
#end
